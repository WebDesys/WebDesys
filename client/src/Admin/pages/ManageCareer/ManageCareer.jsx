...
const ManageCareer = () => {
    ...
    const createJob = async () => {
        setLoading(true)
        try {
            await apiConnector({ method: "POST", url: CareerPage.CreateCareer_API, bodyData: { ...jobData, requirementArray: requirementArray, responsibilityArray: responsibilityArray }, headers: { token: admin.token } })
    ...
    const deleteJob = async (id) => {
        try {
            await apiConnector({ method: "DELETE", url: CareerPage.DeleteCareer_API + `/${id}`, headers: { token: admin.token } })
    ...
    const updateHiringStatus = async (id) => {
        try {
            await apiConnector({ method: "PUT", url: CareerPage.UpdateCareerStatus_API + `/${id}`, headers: { token: admin.token } })
    ...
    useEffect(() => {
        if (admin.role === "Career" || admin.role === "superAdmin" || admin.role === "Admin") {
        }
        else {
                                                ...
                                                <div key={index} className='w-[100%] bg-[#0c3a7c] mt-8' style={{ borderRadius: "80px 0px 130px 50px" }}>
                                                    <div className='flex flex-col items-start  bg-white md:flex-col md:border py-8' style={{ borderRadius: "100px 0px 150px 20px" }}>
                                                        ...
                                                        <div className='flex flex-col justify-between items-center gap-2 md:flex-row'>
                                                            <div>
                                                                {
                                                                    job.active ? (<button className='px-14 py-2 ml-8 bg-[#0c3a7c] text-white rounded-lg text-sm font-[Rubik] mt-8 transition-all duration-300 hover:shadow-lg hover:scale-x-[1.05] ' onClick={() => updateHiringStatus(job._id)}>Activily Hiring</button>) : (<button className='px-14 py-2 ml-8 bg-[#1a3777] text-white rounded-lg text-sm font-[Rubik] mt-8 transition-all duration-300 hover:shadow-lg hover:scale-x-[1.05] ' onClick={() => updateHiringStatus(job._id)}>Hiring Closed</button>)
                                                                }
